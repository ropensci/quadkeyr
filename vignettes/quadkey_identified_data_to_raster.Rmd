...
If you are working with a data.frame with a `quadkey` column, as in the
example image, you can create a raster image from it combining the
QuadKeys' spatial information with any other variable reported.

But before that, consider if you really want a raster image.
You have the option of convert the QuadKeys into a `sf POLYGON data.frame`
with the function `quadkey_df_to_polygon`, and then plot it.
...
If you are sure that you prefer a raster image,
first you must complete the grid with `get_regular_polygon_grid`.
This function gets all the QuadKeys needed to complete the bounding box 
created with the QuadKeys in the column provided. In case you aren't sure
if your grid is complete, you can run the function anyway, 
and it will return the same `sf` data.frame.
The extra QuadKeys added in the process will be `NA`
for all the variables except `geometry`.
After getting the regular grid, 
