...
in `quadkeyr` vignette [From QuadKey to Simple Features data.frame and other conversions](https://fernandez-lab-wsu.github.io/quadkeyr/articles/quadkey_to_sf_conversion.html)
...
For example, let's read this dataset:
...
### Quadkey-identified data to raster images
We aren't certain if we have all the QuadKeys needed to create the raster.
First, let's convert the QuadKeys to polygons and
This function retrieves all the QuadKeys 
required to fill the bounding box 
...
and it will return the same sf data.frame.
The additional QuadKeys added in the process
will have NA values for all columns except `quadkey` and `geometry`


```{r}
complete_polygon_grid <- add_regular_polygon_grid(data = data)

head(complete_polygon_grid$data)
```
As you can see, we obtain a regular polygon grid 
where only the QuadKeys that are reported are colored. 
This is a necessary step towards raster creation. 
The function `add_regular_polygon_grid` returns three outputs in a list: 
the `data`, the `num_cols` and `num_rows`. 
The number of columns and rows will be useful for the next function.
```{r reggrid, out.width= "90%", fig.align='center'}
ggplot() +
...
you can create a [STARS object](https://r-spatial.github.io/stars/articles/stars1.html) 
...
stars_object <- create_stars_raster(data = complete_polygon_grid$data,
                    ...
                    var = "percent_change",
                    nx = complete_polygon_grid$num_cols+1,
                    ny = complete_polygon_grid$num_rows+1)
stars_object
```

```{r rasterimage, out.width= "90%", fig.align='center'}
ggplot() +
...
```
## Advanced use
The function `add_regular_polygon_grid` serves as a wrapper for three functions:
`get_qk_coords`, `regular_qk_grid`, and `grid_to_polygon`.

I will show some of the uses of this functions in isolation:
...
To accomplish this, it necessitates defining the bounding box,
...
to `create_qk_grid` by estimating 
the bounding box from the QuadKeys provided as an argument.
However, if desired, 
you can manually input these arguments and use the function by itself.
...
If you want to convert QuadKeys to polygons, you can use the function
`quadkey_to_df_polygon` as explained
in the basic workflow section of this same vignette.
...
defined study area and zoom level, not all grid QuadKeys are present.
