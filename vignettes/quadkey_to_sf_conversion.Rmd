---
title: "From QuadKey to Simple Features data.frame and other conversions"
output:
  html_document:
    number_sections: true
vignette: >
  %\VignetteIndexEntry{From QuadKey to Simple Features data.frame and other conversions}
...
```{r}
qtll <- quadkey_to_latlong("2103")
qtll
```

```{r}
qtp <- quadkey_to_polygon("2103")
qtp
```

```{r qk, out.width="70%", fig.align='center'}
ggplot() +
...
for each zoom level. In Fig. 1, you can see pixels (0, 0) and (2047, 2047)
for zoom level 3. The `quadkeyr` function `latlong_to_pixelXY`, converts 
...
This function returns the QuadKey string. Since we are estimating 
zoom level 4, the number consists of 4 digits.
...
The function `latlong_to_quadkey` wraps the last three functions,
and converts map coordinates into a QuadKey for a particular zoom level:
...
that wraps the last 3 functions.
...
at the same zoom level. You can try this by yourself.
