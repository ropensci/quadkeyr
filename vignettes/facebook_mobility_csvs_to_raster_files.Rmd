...
knitr::include_graphics("workflow_facebook.png")
```
...
- `read_fb_mobility_files()` produces a warning 
if there are days or times missing among the provided dates in the files.
- The columns `day` and `hour` were created internally.
...
head(files)
```
> Note: The Facebook mobility data used in `quadkeyr` has been altered
> and does not represent real values.
> The examples in this vignette are only to demostrate how functions work,
> not to extract results from this data.
> Please, contact [Data for Good](https://dataforgood.facebook.com/) to
> get datasets.
...
If creating the regular polygon grid using the bounding box of the provided
...
```{r reg_grid, out.width= "70%", fig.align='center'}
ggplot() +
...
the Facebook mobility data in the `files` dataframe.
...
```{r qkgrid, out.width= "70%", fig.align='center'}
...
the number of QuadKeys reported could vary.
...
```{r plotqtll, echo= FALSE, fig.align='center'}
...
```{r polygrid}
polygrid <- grid_to_polygon(data = regular_grid$data)
head(polygrid)
```


```{r plotpolygrid, echo = FALSE, fig.align='center'}
ggplot() + 
  geom_sf(data = ne_countries(returnclass = 'sf'),
          fill = 'beige') +
...
```{r plotploytoras, echo=FALSE, fig.align='center'}
